...
describe('Memory with minimal.z4 fixture', () => {
  ...
  beforeAll(() => {
    ...
    objectFactory = new GameObjectFactory(memory, logger, 4, memory.getWord(HeaderLocation.ObjectTable));
  });
  ...
  describe('Object system', () => {
    it('correctly loads individual objects', () => {
      // V4 uses larger object entries (14 bytes vs 9 bytes in V3)
      // and property defaults table is 63 words (vs 31 in V3)
    ...
    it('correctly identifies object attributes', () => {
      // V4 has 48 attributes (vs 32 in V3)
      const room = objectFactory.getObject(5);
      const box = objectFactory.getObject(6);
      const key = objectFactory.getObject(7);
      ...
      if (key) {
        // Check a few attributes - v4 has 48 attributes
